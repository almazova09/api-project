pipeline {
    ...
    agent {
        kubernetes {
            label "api-project-${BUILD_NUMBER}"
            yaml """
apiVersion: v1
kind: Pod
metadata:
  labels:
    jenkins/label: api-project
spec:
  containers:
    ...
    stages {
        ...
        stage('Run Unit Tests') {
            ...
            post {
                always {
                    script {
                        def xml = findFiles(glob: 'api/test-results/**/*.xml')
                        if (xml.length > 0) {
            ...
            steps {
                container('tools') {
                    sh '''
                        ...
                        njsscan --json --output njsscan-results.json api || true
                    '''
                }
            }
            post {
